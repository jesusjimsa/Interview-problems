...
double timeToArrive(int *dist, int distSize, int speed) {
    int i = 0;
    double time = 0.0;

    for (i = 0; i < distSize - 1; i++) {
        // I use the ceiling function because we have to wait to the next integer hour for the next train
        time += ceil((double)dist[i] / (double)speed);
    }

    // For the last train, we don't need to wait
    time += (double)dist[distSize - 1] / (double)speed;

    return time;
}

int minSpeedOnTime(int* dist, int distSize, double hour) {
    if (ceil(hour) < distSize) {
        // Even if do each distance in one hour, we won't arrive on time
        return -1;
    }
    ...
    while (low < high) {
        ...
        if (timeToArrive(dist, distSize, mid) > hour) {
            low = mid + 1;
        }
        else {
            high = mid;
...
int main(int argc, char const *argv[]) {
    ...
    printf("Minimum speed is %d\n", minSpeedOnTime(trains, sizeof(trains) / sizeof(trains[0]), hour));
