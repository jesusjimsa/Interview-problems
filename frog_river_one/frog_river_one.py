...
def solution(X, A):
    '''
    Return earliest time the frog can jump to the other side.

    Parameters
    ----------
    X : Integer
        Position of the other side of the river.
    A : List
        Times the leaves fall in the river.

    Returns
    -------
    : Integer
        Earliest time the frog can jump to the other side. -1 if not possible.
    '''
    all_steps = list(range(1, X + 1))

    for i in range(X - 1, len(A)):
        if set(all_steps).issubset(A[0:i+1]):
            return i

    return -1


times = [1, 3, 1, 4, 2, 3, 5, 4]

print(solution(5, times))
