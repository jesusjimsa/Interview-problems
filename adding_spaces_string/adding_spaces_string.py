...
class Solution:
    """
    A class providing methods to insert spaces into a string at specified indices.
    """
    def addSpaces(self, s: str, spaces: List[int]) -> str:
        """
        Insert spaces into a string at the specified indices.

        This method modifies the input string by adding spaces at the given indices.
        As spaces are inserted, the indices of subsequent space insertions are adjusted
        to account for previously inserted spaces.

        Args:
            s (str): The original string to add spaces to.
            spaces (List[int]): A list of indices where spaces should be inserted.

        Returns:
            str: A new string with spaces inserted at the specified indices.
        """
        increase = 0

        for i in range(0, len(spaces)):
            s = s[:spaces[i]+increase] + ' ' + s[spaces[i]+increase:]
